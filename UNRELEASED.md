...
- Postgres target now supports Azure Entra ID authentication. A new `--target-postgres-auth-mode` option accepts `Password` (default) or `Entra`. Under `Entra`, PQS registers the `azure-identity-extensions` JDBC authentication plugin and forces `sslmode=require`; the plugin obtains an Azure AD access token via `DefaultAzureCredential`, so the concrete credential (workload identity, client secret, managed identity, ...) is selected by the standard `AZURE_*` environment variables. `--target-postgres-password` becomes optional and is unused under `Entra`; `--target-postgres-username` remains the Postgres role that the Azure AD principal is mapped to.
