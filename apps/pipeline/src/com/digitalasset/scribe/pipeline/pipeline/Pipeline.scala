...
import com.digitalasset.scribe.as
      ...
      (rights, (continueFromOffset, continueFromIx), actualEnd) <- traces.root("initialization routine") {
        for
          ...
          _ <- ZIO.unit @@ traces.attributes(
            "scribe.init.ledger.start"     -> ledgerStart.toString,
            "scribe.init.ledger.end"       -> ledgerEnd.toString,
            "scribe.init.datastore.start"  -> dbStart.toString,
            "scribe.init.datastore.end"    -> dbEnd.toString,
            "scribe.init.normalized.start" -> normalizedStart.toString,
            "scribe.init.normalized.end"   -> normalizedEnd.toString
          )
            ...
            case offset: Offset.Absolute if dbIsEmpty =>
              for
                ...
                _ <- allEvents.run(datastore.processAcs)
                  @@ traces.attributes("scribe.init.acs.offset" -> offset.toString)
          ...
          _ <- ZIO.unit @@ traces.attributes(
            "scribe.init.actual.start" -> continueFromOffset.toString,
            "scribe.init.actual.end"   -> actualEnd.toString
          )
          _ <- logInfo(
