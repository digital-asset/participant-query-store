...
import com.digitalasset.scribe.postgres.document.specific.ValueConverter
...
object model {
  ...
  object Model {
    ...
    def prepareStatement(all: Iterable[Model]): ZIO[ZConnection, Throwable, Chunk[Watermark]] = {
      ...
      def statAttribute(t: Table) = s"scribe.${t.name}.rows_count" -> batchContents.getOrElse(t, 0L)
      all.foreach {
