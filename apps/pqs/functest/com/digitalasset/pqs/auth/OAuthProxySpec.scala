...
object OAuthProxySpec
  ...
  def spec = suite("oauth-proxy")(
    ...
    funcTest("pipeline fails without proxy when OAuth is network-isolated"):
      ...
      And:
        oauthUrl.flatMap(url => Pqs.stderr `is` stringContaining(s"Exception when sending request: POST $url"))
    ,
    ...
    funcTest("pipeline fails when proxy is unreachable"):
      ...
      And:
        oauthUrl.flatMap(url => Pqs.stderr `is` stringContaining(s"Exception when sending request: POST $url"))
      And:
        Pqs.stderr.is(stringContaining("java.net.UnknownHostException: nonexistent-proxy"))
    ,
