...
object HelpSpec extends FuncTestDefault:
  def spec = suite("Help")(
    funcTest("help output by default"):
      ...
      And:
        Scribe.stderr `is` "one of pipeline, datastore, -v, --version expected"
      And:
        Scribe.stdout `is`
          """
...
Run 'scribe COMMAND --help[-verbose]' for more information on a command."""
    ...
    funcTest("help output"):
      ...
      And:
        Scribe.stdout `is` """Usage: scribe COMMAND
...
Run 'scribe COMMAND --help[-verbose]' for more information on a command."""
    ...
    funcTest("verbose output for pipeline command"):
      When:
        Scribe `run` ("pipeline", "--help-verbose")
      ...
      And:
        Scribe.stdout `is` List(
          s"Usage: scribe pipeline SOURCE TARGET [OPTIONS]",
          "",
          "Initiate continuous ledger data export",
          "",
          "Available sources:",
