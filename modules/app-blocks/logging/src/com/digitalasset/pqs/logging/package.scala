  ...
  object Pattern:
    ...
    case object Standard extends Pattern:
      ...
      override def pattern: String =
        s"%label{component}{pqs} %label{instance_uuid}{$instanceUuid} %label{timestamp}{%timestamp{yyyy-MM-dd'T'HH:mm:ss.SSSZ}}" +
          " %label{level}{%level} %label{description}{%message} %label{cause}{%cause} %label{pqs}{%kvs}"
