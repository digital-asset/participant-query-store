...
object PruneCommandSpec extends SharedLedgerAndPostgresTest:
  ...
  def spec = suite("postgres-document prune")(
    funcTest("dry run with valid offset"):
      ...
      When:
        Pqs.runPrune("--prune-target", oneArchived.get.toString)
      Expect:
      ...
      And:
        Pqs.stdout `is` stringContaining("Re-run with --prune-mode Force to execute the pruning operation.")
      And:
    ...
    funcTest("force run with valid offset"):
      ...
      When:
        Pqs.runPrune("--prune-target", oneArchived.get.toString, "--prune-mode", "Force")
      Expect:
        Pqs.exitCode `is` ExitCode.success
      And:
        Pqs.stdout `is` stringContaining(
    ...
    funcTest("dry run fails if using offset out of lower bounds"):
      Given:
        context
      When:
        Pqs.runPrune("--prune-target", smallestOffset.toString)
      Expect:
    ...
    funcTest("dry run fails with latest offset"):
      ...
      When:
        Pqs.runPrune("--prune-target", maxOffset.get.toString)
      Expect:
        Pqs.exitCode `is` ExitCode.failure
      And:
        Pqs.stderr `is` stringContaining(
    ...
    funcTest("dry run fails if using offset out of upper bounds"):
      Given:
        context
      When:
        Pqs.runPrune("--prune-target", biggestOffset.toString)
      Expect:
        Pqs.exitCode `is` ExitCode.failure
      And:
        Pqs.stderr `is` stringContaining(
    ...
    funcTest("force run with timestamp"):
      ...
      When:
        ...
        val timestamp = OffsetDateTime
          ...
          .toString
        Pqs.runPrune("--prune-target", timestamp, "--prune-mode", "Force")
      Expect:
        Pqs.exitCode `is` ExitCode.success
      And:
        Pqs.stdout `is` stringContaining(
    ...
    funcTest("force run with duration"):
      ...
      When:
        Pqs.runPrune("--prune-target", "PT2H", "--prune-mode", "Force")
      Expect:
        Pqs.exitCode `is` ExitCode.success
      And:
        Pqs.stdout `is` stringContaining(
