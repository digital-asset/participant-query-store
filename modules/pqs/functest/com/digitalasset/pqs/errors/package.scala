...
import com.digitalasset.pqs.services.pqs.{CliRun, localPqsDockerImage}
import zio.{ZIO, ZLayer}

package object errors:
  def runPqs(args: String*) =
    ZLayer
      .service[Conf.Conf]
      .project { conf =>
        Docker
  ...
  object Conf:
    ...
    def pg(prefix: String = "") =
      for
        ...
        _prefix = if prefix.isEmpty then prefix else s"${prefix}_".toUpperCase
        base = Map(
          s"PQS_${_prefix}POSTGRES_HOST"       -> pg.container.hostName,
          s"PQS_${_prefix}POSTGRES_PORT"       -> Postgres.port,
          s"PQS_${_prefix}POSTGRES_DATABASE"   -> dbName.name,
          s"PQS_${_prefix}POSTGRES_USERNAME"   -> "postgres",
          s"PQS_${_prefix}POSTGRES_PASSWORD"   -> "postgres",
          s"PQS_${_prefix}POSTGRES_TLS_MODE"   -> "VerifyFull",
          s"PQS_${_prefix}POSTGRES_TLS_KEY"    -> "/tls/client-pg.der",
          s"PQS_${_prefix}POSTGRES_TLS_CERT"   -> "/tls/client-pg.crt",
          s"PQS_${_prefix}POSTGRES_TLS_CAFILE" -> "/tls/root-ca.crt"
        )
      yield (
        base,
        Seq(
    ...
    val failfast = ZIO.succeed((Map("PQS_RETRY_COUNTER_ATTEMPTS" -> "0"), Seq.empty))
